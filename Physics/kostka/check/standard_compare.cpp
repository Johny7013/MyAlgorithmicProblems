...
long usun_biale_znaki(char * buff, long pos) {
...
int main(int argc, char * argv[]) {
	...
	long user_buff_read, orig_buff_read;
	long line;
	long pos;
	...
	do {
		...
		if (user_buff_read != orig_buff_read && user_buff_read+orig_buff_read != -1) {
			printf("0\nLinia %ld: Wczytano %ld znakow, oczekiwano %ld (nie liczac bialych znakow na koncu wiersza)\n", line, user_buff_read, orig_buff_read);
			free(user_buff);
			free(orig_buff);
			fclose(user_output);
			fclose(orig_output);
		...
		if (pos<user_buff_read) {
			printf("0\nLinia %ld, znak %ld: Wczytano '%c', oczekiwano '%c'\n", line, pos+1, user_buff[pos], orig_buff[pos]);
			free(user_buff);
			free(orig_buff);
			fclose(user_output);
			fclose(orig_output);
